...
def get_error_list(error_file_path, select_num=30000):
    error_file = pd.read_csv(error_file_path, sep='\t')
    info_path = pd.read_csv(
        '/data1/seven/workspace/models/cloth/data/train_1225.csv')
    error_info = pd.merge(info_path, error_file, how='left', on='img_name')

    error_dict = Counter(error_info['category_id_level4'])
    sorted_error_dict = dict(
        sorted(error_dict.items(), key=lambda d: d[1], reverse=True))
    keys_10 = list(sorted_error_dict.keys())[:10]
    print('key_type: ', type(keys_10[0]), keys_10)
...
def data_generator(height=224,
    ...
    (trainX, testX, trainY, testY) = train_test_split(imagePaths,
                                                      ...
                                                      test_size=ratio,
                                                      random_state=42)
    trainX = list(trainX) + [imagePaths[-1]]
    testX = list(testX) + [imagePaths[-1]]
    trainY = list(trainY) + [2]
    testY = list(testY) + [2]
...
if __name__ == '__main__':
    ...
    # resnet50_fine_tune = get_pretrained_resnet()
    model_weight_path = '/data1/xusong/shopline_data/train_model_3class_resnet_2w.h5'
    resnet50_fine_tune = get_pretrained_resnet()
    ...
    checkpoint_path = 'logs/resnet_cp.ckpt'
    ...
    resnet50_fine_tune.save('train_model_3class_resnet_2w_add.h5')
