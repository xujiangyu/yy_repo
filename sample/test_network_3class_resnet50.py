...
if __name__ == '__main__':
    ...
    checkpoint_path = 'logs/resnet50_cp.ckpt'
    test_generator, _ = data_generator(imagePaths=imagePaths,
                                       imageLabels=imageLabels,
                                       isTest=True)
    # resnet50_weight_path = '/data1/xusong/shopline_data/train_model_3class_resnet50.h5'
    resnet50_fine_tune.load_weights(checkpoint_path)

    pred_probs = resnet50_fine_tune.predict_generator(test_generator)
    predict_labels = np.argmax(pred_probs, axis=1)

    print(
        tf.math.confusion_matrix(test_generator.classes,
                                 predict_labels,
                                 num_classes=3))