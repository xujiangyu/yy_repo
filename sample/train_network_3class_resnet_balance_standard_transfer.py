...
def get_pretrained_densenet(num_classes=3):
    densenet_fine_tune = keras.models.Sequential()
    densenet_fine_tune.add(
...
def check_image(img_name):
    try:
        ...
        print("error info: ", e, img_name)
        return False


def get_transfered_train_data(transfer_file_3C):
    transfer_data = pd.read_csv(transfer_file_3C, header=0)

    imagePaths = []

    for ind in range(len(transfer_data)):
        tmp_name = transfer_data['img_name'][ind]
        print(tmp_name)
        if check_image(tmp_name):
            imagePaths.append(tmp_name)

    imageLabels = [2] * len(imagePaths)
    print("train data length: ", len(imageLabels))
...
def get_balanced_train_data(shopline_data,
    ...
    # np.savetxt("3C_train.txt", np.array(extend_threeC_paths))
    print(
...
def pixel_standard(image_path):
    rgb_image = Image.open(image_path)
    # rgb_image_stand = rgb_image / 255.0
    rgb2standArr = keras.preprocessing.image.img_to_array(rgb_image) / 255.0

    print("rgb2standArr: ", rgb2standArr.shape)
    return rgb2standArr
...
if __name__ == '__main__':
    ...
    imagePaths1, imageLabels1 = get_balanced_train_data(
        shopline_data, xuanpin_data)
    transfer_file_3C = '/data1/xusong/shopline_data/data/shopline_train_add_others.csv'
    imagePaths2, imageLabels2 = get_transfered_train_data(transfer_file_3C)

    imagePaths, imageLabels = imagePaths1 + imagePaths2, imageLabels1 + imageLabels2

    IS_DENSENET = False
    if IS_DENSENET:
        fine_tune = get_pretrained_densenet()
    else:
        # error_file_path = '/data1/xusong/shopline_data/data/error.txt_2w'
        # imagePaths, imageLabels = get_error_list(error_file_path)
        fine_tune = get_pretrained_resnet()
        # fine_tune.load_weights(
        #     '/data1/xusong/shopline_data/resnet101_balance_standard_add1.h5')
    ...
    # resnet50_fine_tune.submodules()
    # resnet50_fine_tune.load_weights(
    #     "./resnet101v2_weights_tf_dim_ordering_tf_kernels_notop.h5")
    epochs = 5
    log_dir = '/data1/xusong/shopline_classify/logs/train_log/' + datetime.datetime.now(
    ).strftime("%Y%m%d-%H%M%S")
    # Specify the callback object
    tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=log_dir,
                                                          histogram_freq=1)

    checkpoint_path = '/data1/xusong/shopline_classify/logs/train_log/resnet101_balance_standard_transfer1_cp.ckpt'
    ...
    history = fine_tune.fit_generator(
        train_generator,
    ...
    # fine_tune.save('resnet101_balance_standard_add1.h5')
    fine_tune.save('res101_balance_standard_transfer1.h5')
