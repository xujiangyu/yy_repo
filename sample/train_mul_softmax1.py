...
def load_image(img_names, img_dir, img_size=224, is_train=True):
    x = []
    count = 0
    for img_name in img_names:
        img_name = os.path.join(img_dir, img_name)
        if not is_valid_image(img_name):
            x.append(np.zeros([img_size, img_size, 3]))
            count += 1
            continue
        try:
            ...
            if is_train:
                ...
                img = random_crop(img, [img_size, img_size], padding=10)
                img = np.asarray(img)
            else:
...
def load_image_1(img_names, img_dir, img_size=224, is_train=True):
    x = []
    count = 0
    for img_name in img_names:
        img_name = os.path.join(img_dir, img_name)
...
def build_model(
    ...
    inputs_densenet = Input(shape=(img_size, img_size, 3))
    if is_densenet:
    ...
    model.build(input_shape=[(None, max_seq_len), (None, img_size, img_size,
                                                   3)])
...
def main():
    ...
    model_path_old = os.path.join(cur_dir, 'models/model_save_bertresnet_mul')
    ...
    data_file = r"train_1225_add.csv"
    ...
    x_train_img, x_test_img, x_train_text, x_test_text, y_train, y_test, num_classes = \
                    get_train_data(data_dir, data_file, tokenizer, max_seq_len,
    ...
    if IS_INIT:
        latest = tf.train.latest_checkpoint(model_path_old)
        print("old_model:", latest)
