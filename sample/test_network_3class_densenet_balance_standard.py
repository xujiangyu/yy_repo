...
if __name__ == '__main__':
    ...
    random.seed(2021)
    # random.seed(2022)
    imagePaths = random.choices(list(
        pd.read_csv('/data1/xusong/shopline_data/data/shopline_20210108.csv',
                    dtype=str)['img_name']),
                                k=500)
    ...
    model_weight_path = '/data1/xusong/shopline_data/densenet169_balance_standard_add1.h5'

    # resnet50_fine_tune = get_pretrained_resnet()
    fine_tune.load_weights(model_weight_path)

    print("original test labels: ", len(test_datagen.classes))

    predIdxs = fine_tune.predict(test_datagen, batch_size=batch_size)
