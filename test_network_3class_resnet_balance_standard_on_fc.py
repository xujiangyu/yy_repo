...
if __name__ == '__main__':
    ...
    test_datagen, _ = data_generator(imageLabels=imageLabels,
    ...
    model_weight_path = '/data1/xusong/shopline_data/resnet101_balance_standard_add1.h5'
    ...
    fout = open('train_error_sample_101resnet.txt', 'w')
    ...
    # clip_model = resnet50_fine_tune.layers.pop()
    # clip_model.process()
    sub_model = keras.models.Model(
        inputs=resnet50_fine_tune.get_layer('resnet101v2').input,
        outputs=resnet50_fine_tune.get_layer('resnet101v2').output)

    # intermediate_layer_model = keras.Model(
    #     inputs=ResNet50.inputs, outputs=resnet50_fine_tune.layers[0].output)
    new_res = sub_model.predict(test_datagen, batch_size=batch_size)
    print('clip_model res: ', new_res[0])
    np.savetxt('clip_model_res.txt', new_res)

    fout1 = open('test_file_names.txt', 'w')
    for i in range(0, len(test_datagen.filenames), 1000):
        tmp_str = '\t'.join(test_datagen.filenames[i:(i + 1000)])
        fout1.writelines(tmp_str + '\n')
    fout1.close()
